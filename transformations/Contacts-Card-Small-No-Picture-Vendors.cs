{% if (DataItemIndex == 0) {"<div class='row contacts'>"} #%}
    ...
    <div class="border-top-primary card lift mt-4 no-gutters row overflow-hidden">      
      {% IsNullOrEmpty(Picture) ? "" : Format("<div class='d-flex justify-content-around p-3 align-items-center'><div class='overflow-hidden rounded-circle' style='height: 70px;width: 70px;shape-outside: circle();'><img alt='{0}' class='img-cover' src='{1}?width=70' style='height:100%; width:100%; object-position: 50% 0%;' /></div>", Name, GetAttachmentUrlByGUID( Picture, NodeAlias )) #%}
        <div class="d-flex flex-column {% IsNullOrEmpty(Picture) ? "p-3" : "justify-content-center" %}" style="min-height:124px;">
            ...
            {% IsNullOrEmpty(Department) ? "" : Format("<p class='small mt-n2 text-center'>" + Department + "</p>" %}
            {% 
