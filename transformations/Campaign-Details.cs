...
<div class="row">
    ...
    {% if (CampaignValidFrom || CampaignValidTo) {
        IfCompare(
            ...
            "<time class='small text-muted text-right' itemprop='datePublished' datetime='" + FormatDateTime(CampaignValidFrom,"yyyy-MM-dd:HH-mm-ss") + "'>" + 
                IfCompare(
        ...
        <div class="image-container animated flipInX py-3" itemprop="image" itemscope itemtype="http://schema.org/ImageObject">
        {% IsNullOrEmpty(CampaignImage) ? "<img data-src='~/1IM/oneingram-1.9.13/global/default_975.jpg' alt='" + StripTags(CampaignName) + "' class='img-fluid lazyload' />" : 
            Format("<img alt='{0}' class='img-contain lazyload' itemprop='url' data-src='{1}?height=438'>", CampaignName, GetAttachmentUrlByGUID( CampaignImage, NodeAlias )) %}
        </div>
        <div class="py-3" itemprop="articleBody">{% CampaignDescription %}</div>
    </div>
