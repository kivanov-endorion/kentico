...
<li class="nav-item dropdown{% IfCompare(NodeAlias, CurrentDocument.Parent.NodeAlias, "", " active") %}">
...
{% } else {%}
  ...
  <li class="nav-item{% IfCompare(NodeAlias, CurrentDocument.NodeAlias,"", " active") %}{% if(DocumentMenuItemInactive==true){ " disabled" } else {""} %}">
...
<li>
    <a class="align-items-center d-flex dropdown-item justify-content-between" href="{% GetNavigationUrl() %}"{% if(MenuItemTarget != "") {" target='" + MenuItemTarget + "' rel='noopener'"} else {""} %}>
