          ...
          <div>
            {% if ( Documents[NodeALiasPath].Categories) {  %}
              {% foreach (category in Documents[NodeALiasPath].Categories) { 
                "<a href='" + category.CategoryNamePath.ToLower().Replace(" & ","").Replace(" ", "") + "' class='box box-" + category.CategoryDescription.ToLower() + " card-link' title='" + category.CategoryDisplayName + "'>" + category.CategoryDescription + "</a>" 
              } %}
            {% } %}
          </div> 
          {% IfCompare(
  ...
  {% if (DataItemIndex != 0)  { %}
      {%if (DataItemIndex mod 2 == 1)  { "<div class='form-row'>" } %}
        <div class="col-xl-6 col-12 mb-2">
            <div class="form-row">
               <div class="col-3">
                  <div class="card small-card text-center img-rounded shadow-sm wow flipInY h-100" data-wow-delay="{% DataItemIndex * 100 %}ms">
               ...
               <div class="col">
                    <div class="card h-100 shadow-sm">
                      {% if(FormatDateTime(GetDate(DateTime.Today),"yyyy-MM-dd") == FormatDateTime(EventDateStart,"yyyy-MM-dd")) { "<div class='ribbon'><span>{$ 1IM_Today $}</span></div>" } %}
                        <div class="card-body">
                            <a class="card-link" href="{% GetDocumentUrl() %}"><h6 class="mb-0" style="min-height:32px;">{% EventTitle %}</h6></a>
                        </div>
