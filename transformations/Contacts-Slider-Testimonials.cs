...
<div class="card w-75 m-auto shadow">
    <div class="media flex-column flex-lg-row align-items-center">
        <img alt="{% Name %}" class="img-fluid d-none d-lg-inline justify-self-left align-self-start mr-1 " src="{% GetAttachmentUrlByGUID( Picture, NodeAlias ) %}?width=200" />
        <div class="media-body p-4 text-center">
            <h4>{% Name %}</h4>
            <p class="mt-0">{% Speciality %}{% IsNullOrEmpty(Department) ? "" : ", " + Department + %}</p>
            {% Description %}
      </div>
    </div>
</div>
