...
public partial class CMSWebParts_SocialMedia_Yammer_YammerGroupFeed : CMSAbstractWebPart
{
    #region "Properties"
    ...
    public int ContainerHeight
    {
        get
        {
            return ValidationHelper.GetInteger(GetValue("ContainerHeight"), 0);
        }
        set
        {
    ...
    public int ContainerWidth
    {
        get
        {
            //Yammer min is 400 as stated here: https://developer.yammer.com/connect/
            return ValidationHelper.GetInteger(GetValue("ContainerWidth"), 400);
        }
        set
        {
    ...
    public string FeedType
    {
        get
        {
            return ValidationHelper.GetString(GetValue("FeedType"), string.Empty);
        }
        set
        {
            SetValue("FeedType", value);
        }
    }
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            if (PortalContext.ViewMode == ViewModeEnum.LiveSite || PortalContext.ViewMode == ViewModeEnum.Preview)
            {
                ...
                sbDom.AppendFormat("<div id=\"{0}\" style=\"height:{1}px;width:{2}px\"></div>", ContainerName, ContainerHeight, ContainerWidth);
                ...
                sbScript.AppendFormat("   feedType:   \"{0}\",", FeedType);
                sbScript.AppendFormat("   feedId:     \"{0}\"", FeedID);
                sbScript.Append("});");
                sbScript.AppendFormat("</script>");
                
                //render DOM inline
                ltYamCode.Text = sbDom.ToString();
