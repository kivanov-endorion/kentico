...
public partial class CMSWebParts_SocialMedia_Yammer_YammerLikeButton : CMSAbstractWebPart
{
    #region "Properties"
    ...
    public string DataAppID
    {
        get
        {
            return ValidationHelper.GetString(GetValue("DataAppID"), string.Empty);
        }
        set
        {
            SetValue("DataAppID", value);
        }
    }
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            if (PortalContext.ViewMode == ViewModeEnum.LiveSite || PortalContext.ViewMode == ViewModeEnum.Preview)
            {
                ...
                sbScript.Append("   action:   \"like\"");
                sbScript.Append("});");
                sbScript.AppendFormat("</script>");

                //render DOM inline
                ltYamLikeCode.Text = sbDom.ToString();
