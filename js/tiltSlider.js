...
;( function( window ) {
	...
	TiltSlider.prototype._addNavigation = function() {
		...
		for( var i = 0; i < this.itemsCount; ++i ) {
			inner += i === 0 ? '<span class="slider-current"></span>' : '<span></span>';
		}
	...
	TiltSlider.prototype._showItem = function( pos ) {
		...
		classie.removeClass( this.navDots[ this.current ], 'slider-current' );
		...
		classie.addClass( this.navDots[ this.current ], 'slider-current' );
		...
		var cntAnims = 0,
			...
			onEndAnimationCurrentItem = function() {
				++animEndCurrentCnt;
				var endFn = function() {
					classie.removeClass( currentItem, 'slider-hide' );
					classie.removeClass( currentItem, 'slider-current' );
					isFinished();
				};
			...
			onEndAnimationNextItem = function() {
				++animEndNextCnt;
				var endFn = function() {
					classie.removeClass( nextItem, 'slider-show' );
					classie.addClass( nextItem, 'slider-current' );
					isFinished();
				};
		...
		classie.addClass( currentItem, 'slider-hide' );
		classie.addClass( nextItem, 'slider-show' );
