;( function( $, window, document, undefined ) {
	...
	$.extend( Plugin.prototype, {
		init: function() {
			...
			//valid badges
			if( this.settings.stepValidBadge ) {
				this.$steps.each(function(i, el) {
					if ($('[data-acc-title]', el).find(".acc-step-valid").length==0) {
						$('[data-acc-title]', el).append('<span class="acc-step-valid '+mthis.settings.stepValidClass+'"><i class="fa fa-check-circle" aria-hidden="true"></i></span> ');
					}
				})
			}
			...
			if( this.settings.autoButtons ) {
				this.$steps.each(function(i, el) {
					...
					if( i > 0 ) {
						$content.append('<a href="javascript:void(0);" class="'+mthis.settings.autoButtonsPrevClass+'" data-acc-btn-prev data-scroll-ignore>Back</a>');
					}

					// Add next, submit on last
					if( i < ( mthis.$steps.length - 1 ) ) {
						$content.append('<a href="javascript:void(0);" class="'+mthis.settings.autoButtonsNextClass+'" data-acc-btn-next data-scroll-ignore>Next</a>');
...
/*
  		...
  		//var href = athis.$steps.eq(index).find('[data-acc-content]');//this.$(".list-group").find("#goto"+(index+1));

    	//$('.collapse', this.$steps.eq(index)).stop().collapse('show');

    	$('[data-acc-content]', this.element).hide();
     	this.$steps.eq(index).addClass('open').find('[data-acc-content]').show();
     	
      var sp=0;
     	if (index==0) {sp=this.settings.scrollPaddingTop;} else {sp=this.settings.scrollPadding;}
     	
    	$('html, body').animate({
      	scrollTop:$(this.$steps.eq(index)).offset().top-sp
      }, 500);
      ...
      this.currentIndex = index;
    	...
    	$('html, body').animate({
      	scrollTop: $(this.$steps.eq(0).find('[data-acc-content]')).offset().top-(this.stepHeight+this.settings.scrollPaddingTop )
      }, 500);
		}
	});
