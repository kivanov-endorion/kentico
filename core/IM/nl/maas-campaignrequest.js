  ...
  $(document).ready(function(){
     ...
     var ID = $('span[id$="_ItemID_label"]').text();
  
   
 
    var uploadPath="/1IM/oneingram-1.9.13/aspx/pages/dropattachment.aspx";
    //var uploadPath="/1IM/oneingram-1.9.13/aspx/pages/dummy.aspx";
    var refID=ID;
    var fileGroupID="{% IMMacros.GetSessionID() #%}";
    var SourceObject="MaaSRequestCampaign";
    //var table2Update=CMS.Application["breadcrumbsSuffix"].replace("ObjectType.customtableitem_","");
    var table2Update="";
...
$(document).on("click", "#DT_Attachments .button[type='delete']", function (e) {
    ...
    var sUrl = "/content-items/ajax/maas/request-deleteattachmen?ItemID=" + myValue;
...
console.log("SETUP Datatable Attachments");
tblAttachments = $("#DT_Attachments").DataTable({
    ajax: "/content-items/ajax/maas/request-getattachmentlist?SourceObject=" + SourceObject,
    "paging": false,
    "ordering": false,
    "info": false,
    searching: false,
    columns: [
        ...
        },
        {
            ...
            data: "FileName",
            render: function (data, type, row, meta) {
                return '<a href="/1IM/oneingram-1.9.13/aspx/pages/getattachment.ashx?guid=' + row.ItemGUID + '" target="_blank">' + data + '</a>';
            },
        },
        {
            class: "dt-right",
